...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  // Copy the values into buffer arrays
  for (int i = 0; i < 3; i++) {
  ...
  // Convert the values to the determined scale and store the result in the
  // buffer arrays
  ...
  // Add the values in the buffer arrays and store the result in the buf_result
  // array
  s21_data_add(buf_1, buf_2, buf_result, 6);

  // Normalize the result by dividing it by 10 and decreasing the scale until it
  ...
  // Round the result if the last digit is greater than or equal to 5
  if (last_digit >= 5) {
    unsigned carry[6] = {1, 0, 0, 0, 0, 0};
    s21_data_add(buf_result, carry, buf_result, 6);
  }

  // If the result is within the proper range, store it in the result argument
  // and set the scale and sign
  ...
  } else {  // Otherwise, set the error flag
    err = 1 + sign;
  }
  return err;
}
...
int s21_mod(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (value_2.bits[0] == 0 && value_2.bits[1] == 0 && value_2.bits[2] == 0) {
    ...
    s21_to_scale(value_1, 56, buf_1, 9);
    s21_to_scale(value_2, 56, buf_2, 9);
    scale = -1;
    while (s21_data_gt(buf_1, buf_2, 9)) {
      s21_mul_pow10mem(buf_2, 1, 9);
      scale--;
